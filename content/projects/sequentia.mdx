...
And then here I added a loop of exactly what we had before, so we had 10 iterations of gradient descent (forward pass, backward pass and then update). And so running these two cells, initialisation and gradient descent gives us:
...
It gives us some improvement on the loss function. But now I want to use all the words, and there's not 5 but rather 228146 bigrams now, and all the code we wrote doesn't care if theres 5 bigrams or 228146 bigrams, so it should work without any issues.
...
And so we get these results, we are decreasing very slightly so actually we can probably afford a larger learning rate like 1:
...
Ok we can probably afford an even larger learning rate of 10:
...
It's still working! Even 50 seems to work. So let's re-initialise and let's run 100 iterations:
...
Some pretty good losses, let me run 100 more (also what is the number we are expecting?):
...
Kind of anticlimactic but we get the exact same result as we did when we sampled from the counting bigram model, that's because this is the exact same model.
